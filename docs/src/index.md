...
All kinds of simulations are started using the function [`runMC`](@ref). The arguments passed to [`runMC`](@ref) determine which algorithm will be used, number of iterations, the output files, etc.. The signature of the function is:
```julia
runMC(
    pars::Dict,
...
```
The argument `pars` is a dictionary containing a set of generic parameters for the simulation, such as the maximum number of iterations, the name of the simulation, the directory used to write the output, etc. See the documentation of [`runMC`](@ref) for more details. 
...
Currently the following `AbstractMCParams` structures (types) are defined:
  * [`HMCParams`](@ref) for the "standard" HMC algorithm with optional constraints as described in [^4] and [^5];
  * [`NUTSParams`](@ref) for the No U-Turn (NUTS) algorithm as described in [^8];
  * [`ExtMetropParams`](@ref) for the Extended Metropolis algorithm as described in [^9].
...
The full output can be read and saved in an appropriate structure by using the function [`readMCoutput`](@ref). The signature of this function is
```julia
readMCoutput(
    datadir::String,
    simname::String;
...
This package allows you to interrupt the simulation at any time by simply killing the Julia process. That should not create problems to the output files even in case of a crash of the program. The output files can then be used to continue (or re-start) the simulation from the point where it was interrupted by using the function [`contrunMC`](@ref). This function needs as the first argument a dictionary containing the generic simulation parameters (see above). Moreover, this function allows for a continuation using the same exact parameters used before or, by additionally passing a new instance of `AbstractMCParams` of the appropriate type (e.g., a [`HMCParams`](@ref) struct) to continue the previous simulation with new algorithm-specific parameters.
...
Finally, in case the output file containing the saved models gets corrupted because of wrong HDF5 "status flags", the function  [`clear_stflags_hdf5`](@ref) provides a way to clear them and then continue the simulation using [`contrunMC`](@ref).
...
nothing # hide
```
We thus set all the needed parameters for a "standard" HMC simulation, therefore we can instantiate the [`HMCParams`](@ref) structure
```@example gauss1
...
nothing # hide
```
The type of the above structure also defines the algorithm that will be used for sampling, i.e., [`HMCParams`](@ref) implies the use of the classic HMC algorithm, while, e.g., [`NUTSParams`](@ref) implies the use of the NUTS algorithm.
...
We can now start the simulation using the generic function [`runMC`](@ref):
```@example gauss1
pars["stdout"] = "oneline" # hide
runMC(pars,hmcpars,mstart,likelihood=likelihood)
```

We can load the full results of the simulation (using [`readMCoutput`](@ref)), which will store the outputs in a structure of type [`MCSamplers.HMCOutput`](@ref). See the documentation of [`MCSamplers.HMCOutput`](@ref) for an exaplanation of the various fields.
```@example gauss1
...
nothing # hide
```
This time we intend to use the NUTS algorithm, so we start by defining the required parameters for the struct [`NUTSParams`](@ref) as follows
```@example gauss2
...
nothing # hide
```
See the documentation for [`NUTSParams`](@ref) for more details. Now we instantiate a [`NUTSParams`](@ref) structure:
```@example gauss2
...
runMC(pars,hmcpars,mstart,likelihood=likelihood)
```
Load the full results using [`readMCoutput`](@ref)
```@example gauss2
...
mstart = [5.0,-3.0]
```
Now, the structure [`ExtMetropParams`](@ref) for the E-M algorithm need to be instantiated. In this case, such structure requires no parameters to be specified.
```@example gauss3
