...
- [`kli_calcfactors`](@ref KronLinInv.kli_calcfactors): Computes the factors necessary to solve the inverse problem

- [`kli_posteriormean`](@ref KronLinInv.kli_posteriormean): Computes the posterior mean model using the previously computed "factors" with [`kli_calcfactors`](@ref KronLinInv.kli_calcfactors).

- [`kli_blockposteriorcov`](@ref): Computes a block (or all) of the posterior covariance using the previously computed "factors" with [`kli_calcfactors`](@ref KronLinInv.kli_calcfactors).

- [`kli_bandpostcov`](@ref): NOT YET IMPLEMENTED! Computes a band of the posterior covariance the previously computed "factors" with [`kli_calcfactors`](@ref KronLinInv.kli_calcfactors).
...
_The **first** thing to compute is always the set of "factors" using the function [`kli_calcfactors`](@ref). 
Finally, the posterior mean (see [`kli_posteriormean`](@ref KronLinInv.kli_posteriormean)) and/or covariance (or part of it) can be computed (see [`kli_blockposteriorcov`](@ref KronLinInv.kli_blockposteriorcov)).
...
In order to solve the inverse problem using KronLinInv, we first need to compute the "factors" using the function [`kli_calcfactors`](@ref KronLinInv.kli_calcfactors), which takes as inputs two `struct`s containing the covariance matrices and the forward operators.
```@example twodex
...
```@docs
InverseAlgos.KronLinInv
KronLinInv.FwdOps
KronLinInv.CovMats
KronLinInv.KLIFactors
KronLinInv.kli_calcfactors
KronLinInv.kli_posteriormean
KronLinInv.kli_blockposteriorcov
```
