...
"""
    ...
    A single iteration of the NUTS algorithm specific for potential fields problems with polygonal/polyhedral parameterization,
      where a given number of leap-frog iterations are performed.
    """
function oneiterNUTS_poly!(usermodel::NLogPostPDF,HMCpar::NUTSParams,NUTSextrapar::NUTSExtraParams,
    ...
    if isdefined(usermodel.likelihood,:dofwdchecks) && usermodel.likelihood.dofwdchecks && usermodel.likelihood.firstcheck[]
        if usermodel.likelihood.probkind == :polygonalbodies
            pnew = copy(pcur)
        ...
        elseif usermodel.likelihood.probkind == :polyhedralbodies
            checksucceded = usermodel.likelihood(mnew,HMCpar.LcholmassM,"performchecks")
            if checksucceded==false
                error("Initial checks on polyhedra failed. Aborting.")
            end
        else
            error("oneiterNUTS_poly!(): Wrong definition of 'usermodel.likelihood.probkind'. Allowed options are ':polygonalboodies' or 'polyhedralbodies'. Aborting!")
        end    
    end
    ...
    while s == 1
        ...
        if vj == -1
            ...
            θm,pm,_,_,θ1,n1,s1,α,n_α,checksucceded = buildtree_poly(θm,pm,logu,vj,j,ϵ,hamilt_zero,usermodel,HMCpar,grptask,
        ...
        elseif vj == 1
            ...
            ## using log(u) instead of u
            _,_,θp,pp,θ1,n1,s1,α,n_α,checksucceded = buildtree_poly(θp,pp,logu,vj,j,ϵ,hamilt_zero,usermodel,HMCpar,grptask,
                                                                    whichleapfrog=HMCpar.whichleapfrog)
            ##----------------------------------------------------
            ## save current bodyindices
            forwbodyind .= usermodel.likelihood("getbodyindices")
...
"""
function leapfrog_pos_poly(m, p, ϵ, usermodel,HMCpar,grptask)
    ...
    ##======================================================================
    ## CHECKS on the perturbed model (e.g. in case of polygons or polyhedra)
    ##======================================================================
    if isdefined(usermodel.likelihood,:dofwdchecks) && usermodel.likelihood.dofwdchecks
        ...
        #below case usermodel.likelihood.probkind == :polyhedralbodies (check on probkind already performed at the beginning when firstcheck = false
        else
            checksucceded = usermodel.likelihood(mtilde,HMCpar.LcholmassM,"performchecks")
        end
...
"""
function leapfrog_mom_poly(m, p, ϵ, usermodel,HMCpar,grptask)
...
function buildtree_poly(m, p, logu, v, j, ϵ, hamilt_zero, usermodel,HMCpar,grptask ;
    ...
    if j == 0
        ...
        if whichleapfrog==:position
            ...
            m1, p1, checksucceded = leapfrog_pos_poly(m,p,v*ϵ,usermodel,HMCpar,grptask)
        elseif whichleapfrog==:velocity
            ## run the "velocity Verlep" leapfrog algo
            m1, p1, checksucceded = leapfrog_mom_poly(m,p,v*ϵ,usermodel,HMCpar,grptask)
        end
        ...
        mminus,pminus,mplus,pplus,m1,n1,s1,α1,n1_α,checksucceded = buildtree_poly(m,p,logu,v,j-1,ϵ,hamilt_zero, #m0,p0,
        ...
        if s1 == 1
            if v == -1
                mminus,pminus,_,_,m2,n2,s2,α2,n2_α,checksucceded = buildtree_poly(mminus,pminus,logu,v,j-1,ϵ,hamilt_zero, #m0,p0,
                                                                                  usermodel,HMCpar,grptask,
                                                                                  whichleapfrog=whichleapfrog)
                ##===========================================================
                ## If checks fail get out 
                ...
                _,_,mplus,pplus,m2,n2,s2,α2,n2_α,checksucceded = buildtree_poly(mplus,pplus,logu,v,j-1,ϵ,hamilt_zero, #m0,p0,
                                                                                usermodel,HMCpar,grptask,
                                                                                whichleapfrog=whichleapfrog)
                ##===========================================================
                ## If checks fail get out 
